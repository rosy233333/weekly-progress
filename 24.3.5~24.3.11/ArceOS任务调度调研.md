...
为线程实现`Future` `trait`，将上下文切换的代码放入`poll`方法中，与协程一起使用`Executor`进行统一调度。

### 线程的`poll`方法设计

```
在TCB中保存`ra`和`sp`; //恢复寄存器后，CPU的执行流和运行栈都切到了线程自身的，使得正常的`ret`无法返回到`Executor`的执行流
if 第一次运行:
    // 此处还未研究如何启动一个线程
else:
    恢复寄存器;
    return; // 此处是返回到线程自身的执行流开始执行
return Ready<Err<...>>; //unreachable
```

当线程`yield`或`exit`时，先保存线程上下文、重新加入就绪队列（如果是`yield`），再加载TCB中保存的，`Executor`的`ra`和`sp`，然后`return`，以返回`Executor`的执行流
...
`Executor`与线程的切换是否需要保存/恢复栈指针`sp`？

`Executor`需要栈。其本身的代码有多级调用（例如入队出队？），而且协程需要运行在`Executor`的栈上。因此，需要保存/恢复`sp`。

`Executor`与线程的切换是否需要保存/恢复callee saved寄存器？

从上面的简易代码来看，`Executor`在调用`poll`后只需要一个返回值就行，因此可以什么寄存器都不保存。
