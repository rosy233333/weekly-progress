...
kernel-thread对同步和异步任务的处理：kernel-thread创建了系统调用处理协程后，先运行executor直到没有协程就绪，再进入对endpoint的同步等待。因此，基本上不会出现同步阻塞导致异步任务无法执行的情况（只有一种情况：在同步等待endpoint时，异步任务解除阻塞，此时是否无法立刻执行解除阻塞的异步任务？）（需要再次确认）
