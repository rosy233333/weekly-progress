...
我的项目涉及到io_uring机制的**提供**和**使用**两个部分，而在**使用**io_uring的过程中，需要较好地结合协程与io_uring。

在该方向上，已有的参考是Rel4的现有实现。其在发送端直接在工作协程中发送sqe，而在接收端，（无TAIC的情况下）创建了一个dispatcher协程，从CQ中接收结果并通知对应的协程。（有TAIC的情况下）发送方直接通过TAIC唤醒接收方。该方式的不足在于dispatcher协程引入的额外开销，且为了消除该开销需要引入特定的硬件（TAIC）。

tokio引入io_uring的过程也涉及到协程与io_uring的结合。其发送端也是直接在工作协程中发送sqe。而在接收端，其将io_uring的fd注册到了epoll的事件循环中，使用一个线程陷入内核阻塞等待事件发生再处理。在处理完成事件时，获取到协程的Waker并唤醒协程。

应用到异步IPC/异步系统调用的设计中，**阻塞等待**的思路无法使用。但**通过io_uring传递Waker以唤醒协程**的思路可以借鉴。可以设计为：协程的调度循环在每一次循环时从CQ中获取Waker并唤醒对应协程，之后再从就绪队列中取出下一协程并运行。这样也可以达到“在不引入特定硬件的同时取代dispatcher协程”的效果。不过，对于服务端来说，一个协程在正在运行时接收到更多请求应如何处理，还需进一步设计。

（tokio无法摆脱阻塞等待的原因是他们只能在用户态修改，无法修改内核，因此只能使用内核的阻塞等待接口。但我们的工作可以修改内核，因此可以去掉阻塞等待。）
