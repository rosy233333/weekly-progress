...
为了实现工作的模块化，因此本设计只会更改任务数据结构及任务运行机制（增加运行`IPCTask`的机制）。而任务调度模块则可以复用已有的成果。

因此，需要考虑任务调度模块的性质对本设计的影响：

- **轻量级任务**：因为每一条IPC请求都会被转化为一个任务，因此为了降低开销，任务调度模块需要支持轻量级任务，例如协程或用户级线程。同时，需要以轻量级任务为单位阻塞，才能实现内核级线程的非阻塞。
- **用户态与内核态的统一调度**：为了实现异步系统调用，需要对用户态和内核态的调度器均进行修改。如果二者实现了统一调度，则能降低修改的工作量。但该特性不是必需的，只需要内核态和用户态的调度器向对方暴露添加任务的接口即可。
- **进程、线程和协程的统一调度**：在该设计下，不同的进程共享同一条任务队列。因此，“放入接收方进程的任务队列”的行为变为了“打上接收方进程的标记后放回任务队列”。两者可兼容并可发挥各自的优势。
- **优先级与抢占式调度**：可以进一步提高高优先级任务的实时性。同时，本设计可以使IPC消息带有的优先级继承发送方的优先级，避免了IPC导致的优先级反转现象（高优先级任务等待低优先级任务回复IPC消息）。
